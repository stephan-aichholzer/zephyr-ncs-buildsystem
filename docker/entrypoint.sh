...
cmd_test() {
    ...
    [[ -f "$tests_root/CMakeLists.txt" ]] || {
        err "no top-level CMakeLists.txt at $tests_root"; exit 2;
    }

    local build_root="/tmp/test-build"
    mkdir -p "$build_root"
    [[ -d /artifacts ]] && mkdir -p /artifacts/tests

    info "configure: $tests_root -> $build_root"
    cmake -GNinja -B "$build_root" -S "$tests_root" \
          -DZEPHYR_BASE=/opt/ncs/zephyr

    info "build: ExternalProject sub-builds for every test"
    cmake --build "$build_root"

    info "run: executing each ztest_* binary"
    local pass=0 fail=0 total=0
    shopt -s nullglob
    for exe_dir in "$build_root"/ztest_*; do
        [[ -d "$exe_dir" ]] || continue
        local test_name exe dest log_file
        test_name="$(basename "$exe_dir")"
        exe="$exe_dir/$test_name"
        [[ -x "$exe" ]] || continue
        ...
        info "--- $test_name ---"
        if "$exe" 2>&1 | { [[ -d /artifacts ]] && tee "$log_file" || cat; }; then
            [[ -d /artifacts ]] && cp "$exe" "$dest/"
            pass=$((pass + 1))
            info "  PASS: $test_name"
        else
            fail=$((fail + 1))
